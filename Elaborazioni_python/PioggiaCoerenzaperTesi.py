...
CoerVH = pd.read_csv("D:\Progetti_python\Verona\Sentinel1\Databases_tutti\VH_Sentinel1_Coerenza.csv", sep=";", index_col="Id", decimal=",")
CoerVV = pd.read_csv("D:\Progetti_python\Verona\Sentinel1\Databases_tutti\VV_Sentinel1_Coerenza.csv", sep=";", index_col="Id", decimal=",")
...
tableVH = CoerVH.drop("DE_VARIETA", axis=1)
tableVV = CoerVV.drop("DE_VARIETA", axis=1)
...
x_asse = [datetime.datetime.strptime(item[0:10], "D_%Y%m%d") for item in x_asse[0:len(x_asse)]]
...
plt.ylabel('Coerenza VH', fontsize=10)
...
ax1.plot(x_asse,rowBackVV, 'g-o', color= "black", label= "Coerenza")
ax1.grid(which='major', linestyle='-.', linewidth='0.1', color='black')
plt.ylabel('Coerenza VV', fontsize=10)
...
ax3.legend(bbox_to_anchor=(1.04, 0.9), loc='upper left', borderaxespad=0.)
